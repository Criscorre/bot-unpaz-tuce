# faq_data.py — Preguntas frecuentes de la TUCE
# Agregar nuevas preguntas acá sin tocar el código del bot.

FAQ = [
    {
        "q": "¿Cuándo empiezan las clases?",
        "a": (
            "📅 *Inicio de clases:*\n"
            "• 1er trimestre: 9 de marzo 2026\n"
            "• 2do trimestre: 10 de agosto 2026"
        ),
    },
    {
        "q": "¿Cómo me inscribo a materias?",
        "a": (
            "✍️ *Inscripción a materias:*\n"
            "Por SIU Guaraní en las fechas publicadas.\n"
            "🔗 https://estudiantes.unpaz.edu.ar/autogestion/"
        ),
    },
    {
        "q": "¿Cómo tramito el boleto estudiantil?",
        "a": (
            "🚌 *Boleto estudiantil:*\n"
            "Gestionalo vía SIU Guaraní siendo alumno regular.\n"
            "Consultas: soporteinscripciones@unpaz.edu.ar\n"
            "🔗 https://estudiantes.unpaz.edu.ar/autogestion/"
        ),
    },
    {
        "q": "¿Dónde está el campus virtual?",
        "a": (
            "🎓 *Campus virtual:*\n"
            "🔗 https://campusvirtual.unpaz.edu.ar/"
        ),
    },
    {
        "q": "¿Cómo solicito una beca?",
        "a": (
            "🎓 *Becas y tutorías:*\n"
            "Contactá: accesoapoyo@unpaz.edu.ar\n"
            "Más info: https://unpaz.edu.ar/bienestar/becas"
        ),
    },
    {
        "q": "¿Cómo tramito equivalencias?",
        "a": (
            "⚖️ *Equivalencias:*\n"
            "Período: 01/04 al 10/04/2026\n"
            "Formulario: https://unpaz.edu.ar/formularioequivalencias"
        ),
    },
    {
        "q": "¿Cuáles son las sedes?",
        "a": (
            "📍 *Campus Alem:*\n"
            "• CEM (Pueyrredón): Av. Alem 4593\n"
            "  🗺️ https://maps.google.com/?q=Leandro+N.+Alem+4593,+Jose+C.+Paz\n"
            "• Sede Alem: Av. Alem 4731\n"
            "  🗺️ https://maps.google.com/?q=Leandro+N.+Alem+4731,+Jose+C.+Paz\n"
            "• Biblioteca: Av. Alem 4673\n"
            "  🗺️ https://maps.google.com/?q=Leandro+N.+Alem+4673,+Jose+C.+Paz\n\n"
            "📍 *Campus Arregui:*\n"
            "• Sede Arregui: Av. Arregui 501\n"
            "  🗺️ https://maps.google.com/?q=Av.+Hector+Arregui+501,+Jose+C.+Paz"
        ),
    },
    {
        "q": "¿Cuánto dura la carrera?",
        "a": (
            "🎓 *Duración:* 3 años (6 trimestres)\n"
            "Modalidad: Presencial con contenido virtual.\n"
            "Título: Técnica/o Universitaria/o en Comercio Electrónico."
        ),
    },
    {
        "q": "¿Cómo obtengo un certificado de alumno regular?",
        "a": (
            "📄 *Certificados:*\n"
            "Se emiten de forma digital vía SIU Guaraní.\n"
            "🔗 https://estudiantes.unpaz.edu.ar/autogestion/"
        ),
    },
    {
        "q": "¿Tiene salida laboral la carrera?",
        "a": (
            "💼 *Salida laboral TUCE:*\n"
            "• Marketing digital y e-commerce\n"
            "• Gestión de tiendas online\n"
            "• Estrategia web y SEO\n"
            "• Emprendimiento digital\n"
            "• Consultoría en negocios online\n"
            "• CEO de emprendimientos propios\n"
            "🔗 https://unpaz.edu.ar/comercioelectronico"
        ),
    },
    {
        "q": "¿Cómo tramito una pasantía?",
        "a": (
            "💼 *Pasantías:*\n"
            "Más info en: https://unpaz.edu.ar/pasantias\n"
            "Consultas: consultasestudiantes@unpaz.edu.ar"
        ),
    },
    {
        "q": "¿Qué es el SIU Guaraní?",
        "a": (
            "🖥️ *SIU Guaraní — gestión académica:*\n"
            "🔗 https://estudiantes.unpaz.edu.ar/autogestion/\n\n"
            "Desde ahí podés:\n"
            "• Inscribirte a exámenes y materias\n"
            "• Ver tu historia académica e inasistencias\n"
            "• Consultar tu plan de estudios y avance\n"
            "• Tramitar constancias y certificados\n"
            "• Solicitar el boleto estudiantil\n"
            "• Actualizar tus datos personales\n"
            "• Iniciar trámites de título y preinscripción"
        ),
    },
    {
        "q": "¿Cómo llego a UNPAZ?",
        "a": (
            "🗺️ *Cómo llegar a UNPAZ — Campus Alem:*\n\n"
            "Tocá el link según tu medio de transporte y Maps te guía desde donde estás 👇\n\n"
            "🚌 Colectivo / tren:\n"
            "https://www.google.com/maps/dir/?api=1&destination=Leandro+N.+Alem+4731,+Jose+C.+Paz,+Buenos+Aires&travelmode=transit\n\n"
            "🚗 Auto / moto:\n"
            "https://www.google.com/maps/dir/?api=1&destination=Leandro+N.+Alem+4731,+Jose+C.+Paz,+Buenos+Aires&travelmode=driving\n\n"
            "🚶 A pie:\n"
            "https://www.google.com/maps/dir/?api=1&destination=Leandro+N.+Alem+4731,+Jose+C.+Paz,+Buenos+Aires&travelmode=walking"
        ),
    },
    {
        "q": "¿Quién es el director de la carrera?",
        "a": (
            "👤 *Director de la TUCE:*\n"
            "Fernando Fiorenzo\n"
            "🔗 https://www.linkedin.com/in/fernando-fiorenzo-0a47841a/"
        ),
    },
]
